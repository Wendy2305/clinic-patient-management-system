...
import { transformEntriesToDateSections } from '../../services/biodata-entry.service';
    ...
    const searchFilterFunction = useCallback((text: string) => {
        ...
        if (text) {
            ...
            const newData = assetEntries.filter((item) => {
                ...
                const combinedFields = ` ${item.acquireMonth + 1} ${moment([item.acquireYear, item.acquireMonth, item.acquireDay]).format("LL")} ${item.FName} ${item.SName} ${item.MName} ${item.DOBi}${item.Hadd} ${item.DateReg} ${item.Matno}`;
