...
const Spreadsheet: React.FC<Props> = ({ entries }) => {
    ...
    const makeTable = () => {
        ...
        const table = {
            ...
            headers: entriesToShare.length > 0? ["", "S/N", "Date", "Clinic Date", "Name of Ailment", "Meds", "Proc Under", "Date of Next App"] : [],//first empty space is for id in row which is not shown but kept for reference
            rows: entriesToShare.map((entry) => {
                return [entry.id, entry.SN, moment([entry.txnYear!, entry.txnMonth!, entry.txnDay!]).format("LL"), entry.ClinicDate, entry.Nail, entry.Meds, entry.ProcUnder, entry.DateApp, ]
            }),
