    ...
    const searchFilterFunction = useCallback((text: string) => {
        ...
        if (text) {
            ...
            const newData = transactionEntries.filter((item) => {
                ...
                const combinedFields = `${item.txnMonth + 1} ${moment([item.txnYear, item.txnMonth, item.txnDay]).format("LL") } ${item.ClinicDate} ${item.Nail}  ${item.Meds} ${item.ProcUnder} ${item.DateApp}`;
